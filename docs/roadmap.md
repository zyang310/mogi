...
- [x] Session-start "ready" state — the moment right after Start (before the first message) shows a composed empty state (`session/ChatEmptyState`): breathing icon, "Ready when you are" prompt, and three quick-start chips ("I'm ready to start" / "Walk me through the problem" / "Give me a hint") that send a canned first message through the normal `SendMessage` path, plus a clearer "watching your screen" capture indicator. Regular Hub sessions only — company/mock sessions open on their AI opener, so they never hit the empty state.
- ◑ Keyboard shortcuts — **global voice hotkey built**: press a configurable hotkey (default `Right ⌥ Option`) to start recording, press again to stop & send — works while the IDE is focused. Backend OS hook (`internal/hotkey` via `robotn/gohook`) → `ptt:down`/`ptt:up` Wails events (frontend toggles on down); config + macOS Accessibility hint in Settings → Voice Hotkey. End-session / toggle-capture shortcuts still TODO.
...
- [x] **Managed test accounts** — testers redeem an **invite code + email OTP** and the app installs developer-funded keys itself, so nobody needs their own OpenRouter/Google/ElevenLabs key. **BYOK stays a first-class equal mode** (two key namespaces + a `KeyMode` preference; switching either way never disturbs the other set). Model is pinned server-side so a cohort's feedback is comparable; voice is Google-only in managed mode (the shared ElevenLabs key is STT-scoped). A small Go **access service** (`access-service/`, Cloud Run + Firestore + Secret Manager + Resend) mints a **$3-capped OpenRouter key per tester** and re-serves it on each launch; it hands out keys but **never proxies interview traffic**, so the privacy story is unchanged. Console-editable **kill switch** + per-tester revocation, verified live. Bindings: `RequestTestCode` / `ActivateTestAccount` / `SignOutTestAccount`. See [managed-keys-plan.md](managed-keys-plan.md) (design) and [managed-keys-implementation.md](managed-keys-implementation.md) (build log).
- [ ] OpenRouter OAuth PKCE flow — still the answer for **BYOK** friction (key acquisition in two clicks, user pays OpenRouter directly); orthogonal to the managed tier above
