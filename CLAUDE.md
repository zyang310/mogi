...
| `internal/service/` | **Business logic (the service layer)** — one file per concern: `interview.go` (active-session state + mutex, send loop, company + set-mock starts, post-session meta extraction), `history.go` (transcripts, delete guard, debrief cache policy), `voice.go` (STT/TTS provider resolution + fallbacks), `settings.go` (keys + prefs, incl. propagation to capturer/hotkey), `sets.go` (custom question-set CRUD + validation), `account.go` (managed test accounts: activate/sign-out/launch-refresh + `applyKeyMode`, the single key-resolution rule), `providers.go` (RWMutex registry of live API clients + the `AI`/`TTS`/`STT`/`Speech` client interfaces; each service colocates its own narrow store-interface slice). Unit-tested against fakes (`fakes_test.go`) — no SQLite, no network, no OS hooks. |
...
| `internal/store/` | SQLite (`data.db`): sessions + messages (transcripts), preferences, API keys, custom question sets. Session-history reads/writes (`ListSessions`, `GetSessionTranscript`, `UpdateSessionMeta`, `DeleteSession`) live in `sessions.go`; `question_sets.go` stores custom sets (full problem snapshots in a JSON column, so sets survive dataset refreshes). `managed.go` adds the **managed key namespace** (`managed_*` rows) on the same pref primitives — no schema change, so BYOK and managed keys coexist untouched. |
...
- [docs/question-sets-plan.md](docs/question-sets-plan.md) — **custom question sets**: per-company curated draw pools (snapshot storage, JSON-column decision, min-2/no-recent-filter draw; implemented)
